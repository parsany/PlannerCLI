...
import json
import os
from modules import endeavor, schedule
...
def calculate_activity_rating():
    try:
        ...
        # Read the "timejc.json" file
        with open(TIMEJC, 'r') as file:
            tasks = json.load(file)

        # Filter tasks completed today
        today_date = datetime.date.today().strftime('%Y-%m-%d')
        today_tasks = [task for task in tasks if task['completed_at'] == today_date]
        total_hours_today = sum(task['time_taken'] for task in today_tasks)
        
        total_tag = float(schedule.show_tags(showl=True))

        if total_hours_today == 0:
            completion_percentage = 0
        else:
            completion_percentage = ((total_hours_today/60) / (total_tag/7)) * 100
        ...
        print(f"{colors.space*7}{colors.RED}The TIMEJC file was not found.{colors.RESET}")
    except json.JSONDecodeError:
        print("Error reading JSON data from TIMEJC. Please check the file format.")
...
def check_and_update_yesterday_activity():
    ...
    if yesterday_str not in activity_data:
        ...
        try:
            ...
            print(f"{colors.space*7}{colors.RED}The ENDV_FILE file was not found, Please make sure to save rouptilsnes.{colors.RESET}") 
