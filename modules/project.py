...
def list_studies(sort=False):
    if len(studies) == 0:
        print("No studies added yet.")
    else:
        if sort == True:
            sorted_studies = sorted(studies, key=lambda x: x['days'], reverse=True) 
        else:
            sorted_studies = studies
        total_days = sum([project['days'] for project in sorted_studies]) 
        total_multiplier = sum([project['multiplier'] for project in sorted_studies])
        print(f"\n{'Index':<6}{'Name':<39}{'Days':<20}{'Multiplier':<20}{'Done':<10}{'selected':<10}")
        print("----------------------------------------------------------------------------------------------------------")
        for project in sorted_studies:
            if project['selected'] == 1:
                colorsb = colors.YELLOW
            else:
                colorsb = colors.CYAN
            print(f"{colorsb}{project['index']:<6}{project['name']:<39}{project['days']:<20.2f}{str(project['multiplier']):<20}{project['done']:<12}{project['selected']:<13}{colors.RESET}")
        print("----------------------------------------------------------------------------------------------------------")
