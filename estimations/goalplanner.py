...
TODAY_JSON = os.path.join(colors.notes_file, 'todaysdate34.json')
TRACKER_DB = os.path.join(colors.notes_file, 'goalprime.json')
UNACTIVE_DB = os.path.join(colors.notes_file, 'unactivegoals.json')
TPRIME_DB = os.path.join(colors.notes_file, 'gtaskprime.json')
NPRIME_DB = os.path.join(colors.notes_file, 'goalprime.json')
pomofile = os.path.join(colors.pomo_file, 'taskmaing.txt')
...
def tracker_greedy(rint=False):
    if len(trackers) >0:
        ...
        if len(merged_trackers)>2:
            ...
            if rint:
                print(f"\n\n{colors.space*5}{colors.YELLOW}Goals for the next {newt[0][1]} days:{colors.RESET}")
            m = 1
...
def main():
    while True:
        load_trackers()
        load_tasks()
        print(f"\n{colors.YELLOW}||Goal Planner||{colors.RESET}\n\n{colors.GREEN}Menu:{colors.RESET}")
