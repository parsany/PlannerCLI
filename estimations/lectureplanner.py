...
FTODAY_JSON = os.path.join(colors.notes_file, 'todaysdatelec.json')
FTRACKER_DB = os.path.join(colors.notes_file, 'lecturetracker.json')
FUNACTIVE_DB = os.path.join(colors.notes_file, 'unactivetrackersv.json')
FTPRIME_DB = os.path.join(colors.notes_file, 'lecturetask.json')
pomofile = os.path.join(colors.pomo_file, 'lecture.txt')
...
def calculate_tracker_totals():
    filtered_trackers = [tracker for tracker in trackers if tracker['name'] != "."]
    total_amount = sum(tracker['amount'] for tracker in filtered_trackers)
    total_done = sum(task['amount'] for task in tasks if task['tracker'] in [tracker['name'] for tracker in filtered_trackers])
    return total_amount, total_done

def tracker_greedy(rint=False):
    if len(trackers) >0:
        sorted_trackers = []
        shared_indices = []
        merged_trackers = [[0,0]]
        ...
        if len(merged_trackers)>2:
            ...
            if rint:
                print(f"\n{colors.space*5}{colors.YELLOW}Lecture Trackers in next {newt[0][1]} days:{colors.RESET}")
            m=1
            ...
            for element in trackername:
                ...
                if m <= len(time_left_list):
                    ...
                    break
            if rint:
                minute = day_list[0]%60
                hour = (day_list[0]-minute)/60 
                print(f"{colors.space*5}{colors.YELLOW}tracker per day: {day_list[0]:.2f} minutes [{hour:.0f}:{minute:.0f}] {colors.RESET}")
            video_var = day_list[0]
        else:
            if rint:
                    print("ERROR: Merge conflict type [VOD]")
    else:
        video_var=0
        if rint:
            print(f"\n\n{colors.space*3}{colors.BLUE}no trackers found.{colors.RESET}")
    return video_var



def data_greedy(rint):
    if len(trackers) >0:
        sorted_trackers = []
        shared_indices = []
        merged_trackers = [[0,0]]
...
def add_task_from_file():
    ...
    if not tasks_from_file:
        ...
        return
    print("\n\nNew Lectures found! wanna add them?")
...
def main():
    while True:
        load_trackers()
        load_tasks()
        print(f"\n{colors.YELLOW}||Lecture Planner||{colors.RESET}\n\n{colors.GREEN}Menu:{colors.RESET}")
